...
impl<T> Term<T> {
    ...
    fn grid_to_string_between(&self, y0: Line, y1: Line) -> String {
        ...
        for y in (*y0..*y1).rev() {
            ...
            if !row[self.grid.num_cols() - 1].flags().contains(Flags::WRAPLINE) {
                s.push('\n');
            }
        }
        s
    ...
    pub fn grid_to_string_only_visible(&self) -> String {
        self.grid_to_string_between(Line(self.grid.display_offset()), self.grid.num_lines())
    }

    pub fn grid_to_string(&self) -> String {
        self.grid_to_string_between(
            Line(0),
            Line(*self.grid.num_lines() + self.grid.history_size()),
        )
    }
}
...
mod tests {
    ...
    fn to_string() {
        ...
        term.grid.scroll_up(&(Line(0)..Line(4)), Line(1), &Cell::default());
