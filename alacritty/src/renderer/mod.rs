...
pub struct GlyphCache {
    ...
    /// Binary content of fonts.
    font_contents: Vec<Box<[u8]>>,

    /// A map of FontKey to allsorts Font.
    allsorts_fonts: HashMap<
        FontKey,
        allsorts::Font<allsorts::font_data::DynamicFontTableProvider<'static>>,
        BuildHasherDefault<FnvHasher>,
    >,

    /// features set depending on ligatures option.
    font_features: allsorts::gsub::Features,
}
...
impl GlyphCache {
        ...
        L: LoadGlyph,
    {
        ...
        let features = if config.font.ligatures {
            Features::Mask(FeatureMask::CALT | FeatureMask::LIGA)
        } else {
            Features::Mask(Default::default())
        };
        ...
        let mut cache = Self {
            ...
            metrics,
            font_contents: Vec::new(),
            allsorts_fonts: HashMap::default(),
    ...
    where
        L: LoadGlyph,
    {
        if let Entry::Vacant(v) = self.allsorts_fonts.entry(font_key) {
            let path = self.rasterizer.font_path(font_key)?;
            let file_content = std::fs::read(path)?.into_boxed_slice();
            self.font_contents.push(file_content);
            let scope = ReadScope::new(self.font_contents.last().unwrap());
            let font_data = scope.read::<FontData<'_>>()?;
            let table = font_data.table_provider(0)?;

            let font = unsafe {
                // SAFETY: extend Font<DynamicFontTableProvider<'_>> to
                // Font<DynamicFontTableProvider<'static>>. This references to the
                // memory managed font_contents and it works as long as the font is not moved out.
                std::mem::transmute(allsorts::Font::new(table)?.unwrap())
            };
            v.insert(font);
        };

        let font = self.allsorts_fonts.get_mut(&font_key).unwrap();
        let glyphs = font.map_glyphs(text_run, tag::LATN, MatchingPresentation::NotRequired);
        let infos =
            font.shape(glyphs, tag::LATN, None, &self.font_features, false).map_err(|(e, _)| e)?;
        Ok(infos
            .into_iter()
            .map(move |glyph_info| {
                let codepoint = glyph_info.glyph.glyph_index;

                // Codepoint of 0 indicates a missing or undefined glyph
                let id: crossfont::GlyphId = if codepoint == 0 {
                    GlyphId::char(*glyph_info.glyph.unicodes.first().unwrap())
                } else {
